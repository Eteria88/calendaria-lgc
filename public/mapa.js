// Mapa y Códigos (UIT) — panel de búsqueda + My Maps (embed)
// - El mapa lo administra la Red en Google My Maps (iframe).
// - Este JS mantiene la herramienta de búsqueda de códigos de país.
...
(function(){
  ...
  const DEFAULT_DATA_URL = "countries_embed.json";

  async function getEmbedArray(){
    const url = (window.MAPA_DATA_URL || DEFAULT_DATA_URL);
    if (!url) return EMBED;
    try{
      ...
      console.warn("[Mapa] No pude cargar dataset remoto, uso fallback embebido.", err);
      return EMBED;
    }
  }
  ...
  async function init(){
    listEl = $("#country-list");
    searchEl = $("#search");
    badgeEl = $("#search-code-badge");
    searchCodeEl = $("#search-code");
    codeNameBadge = $("#search-code-name");
  ...
  async function loadEmbed(){
    const arr = await getEmbedArray();
    for (const x of arr){
      const raw = (x && (x.n3 ?? x.N3 ?? x.id)) ?? "";
      const n3 = String(raw).padStart(3,"0");
      if(!n3 || n3 === "000") continue;
      const name = (x.name || x.country || n3);
      const phone = (x.phone || x.code || x.dial || "");
      state.phoneByN3[n3] = phone;
      state.nameByN3[n3]  = name;

      if (name) state.phoneByName[normText(name)] = phone;
      if (phone){
        const nc = normCode(phone);
        state.n3ByPhone[nc] = state.n3ByPhone[nc] || n3; // conserva el primero
      }
    }
  }

  function buildIndex(){
    state.idx = Object.keys(state.nameByN3)
      .map(n3 => ({ n3, name: state.nameByN3[n3] }))
      .sort((a,b)=> a.name.localeCompare(b.name, "es", { sensitivity:"base" }));
  }

  function renderList(list=null){
    const LST = list || state.idx;
    if (!listEl) return;
    listEl.innerHTML = "";
    for(const item of LST){
      ...
      li.innerHTML = `<span>${escapeHtml(item.name)}</span><span class="country-code">${escapeHtml(phone || "—")}</span>`;
      li.onclick = ()=> selectByN3(item.n3);
      listEl.appendChild(li);
    }
  }

  function selectByN3(n3){
    const name = state.nameByN3[n3] || `N3 ${n3}`;
  ...
  function highlightList(n3){
    ...
    items.forEach(li => li.classList.toggle("is-active", li.dataset.n3 === String(n3)));
  }
  ...
  function setupSearchByName(){
    ...
    searchEl.addEventListener("keydown",(ev)=>{
      if(ev.key === "Enter"){
        ...
        const q = searchEl.value.trim().toLowerCase();
        if(!q) return;
        const match = state.idx.find(x => x.name.toLowerCase() === q) ||
                      state.idx.find(x => x.name.toLowerCase().startsWith(q)) ||
                      state.idx.find(x => x.name.toLowerCase().includes(q));
        if(match) selectByN3(match.n3);
      }
    });
  }
