(function(){
  ...
  try{
    ...
    function sortMarcasLgc(){
      var grid=document.getElementById('marcasLgcGrid');
      if(!grid) return;
      var cards=[].slice.call(grid.querySelectorAll(':scope > .a-card'));
      function parseISO(s){
        if(!s) return null;
        // Fuerza UTC para evitar corrimientos por zona horaria
        var d=new Date(s+'T00:00:00Z');
        return isNaN(d.getTime())?null:d.getTime();
      }
      cards.sort(function(a,b){
        var ta=parseISO(a.getAttribute('data-date-iso'));
        var tb=parseISO(b.getAttribute('data-date-iso'));
        if(ta===null) ta=9e18;
        if(tb===null) tb=9e18;
        return ta-tb;
      });
...
function fmtDate(d){
    ...
    // Marcas LGC: calcula el conteo en días (según la fecha de referencia) para TODAS las tarjetas
    // que estén dentro del grid, usando su data-date-iso.
    function updateMarcasLgcCounts(ref){
      var grid=document.getElementById('marcasLgcGrid');
      if(!grid) return;
      ...
      cards.forEach(function(card){
        var out=card.querySelector('.mono');
        if(!out) return;
